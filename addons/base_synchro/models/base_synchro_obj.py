...
class BaseSynchroObj(models.Model):
    ...
    name = fields.Char('Name', index=1, required=True)
    domain = fields.Char('Domain', index=1, required=True, default='[]')
    server_id = fields.Many2one('base.synchro.server', 'Server',
                                ondelete='cascade', index=1, required=True)
...
class BaseSynchroObjAvoid(models.Model):
    ...
    name = fields.Char('Field Name', index=1, required=True)
