...
class SaleOrder(models.Model):
	...
	synced_weighbridge_server_ids = fields.One2many('sale.order.weighbridge.server', 'order_id', 'Synced Weighbridge Server')
	...
	def update_synced_order(self, server_key, synced_order_ids):
		for pid in synced_order_ids:
			self.env['sale.order.weighbridge.server'].create({
					'order_id': pid,
					'name': server_key,
					'synced': True,
				})
...
class SaleOrderWeighbridgeServer(models.Model):
	_name = 'sale.order.weighbridge.server'

	name = fields.Char('Server Key', required=True)
	order_id = fields.Many2one('sale.order', 'Order Ref', required=True, ondelete='cascade')
