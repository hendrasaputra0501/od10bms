...
class TuitionBilling(models.TransientModel):
    ...
    line_ids = fields.One2many('bm.tuition.bill.line', 'wizard_id', string="Bills")
        ...
        for unit in self.env['bm.unit'].search(unit_domain):
            ...
            if self.service_charge:    
                ...
                for line1 in tuitions_open1:
                    self.env['bm.tuition.bill.line'].create({
                        'wizard_id': self.id,
                        'unit_id': unit.id,
...
class TuitionBillingLines(models.TransientModel):
    _name = 'bm.tuition.bill.line'
    _description = 'Bill Lines'

    wizard_id = fields.Many2one('wizard.bm.tuition.bill', string="Wizard")
