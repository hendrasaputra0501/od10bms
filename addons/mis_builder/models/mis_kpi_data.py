...
class MisKpiData(models.AbstractModel):
    ...
    def _intersect_days(self, dt_from, dt_to):
        item_dt_from = fields.Date.from_string(self.date_from)
        item_dt_to = fields.Date.from_string(self.date_to)
    ...
    def _query_kpi_data(self, date_from, date_to, base_domain):
        ...
        for item in self.search(domain):
            i_days, item_days = item._intersect_days(dt_from, dt_to)
            if item.kpi_expression_id.kpi_id.accumulation_method == ACC_SUM:
                ...
                res_avg[item.kpi_expression_id].append(
                    (i_days, item.amount))
            else:
                raise UserError(
