...
class TestMisReportInstanceDataSources(common.TransactionCase):
    ...
    def setUp(self):
        ...
        self.p1 = self.env['mis.report.instance.period'].create(dict(
            name='p1',
            report_instance_id=self.instance.id,
            manual_date_from='2017-01-01',
            manual_date_to='2017-01-31',
        ))
        self.p2 = self.env['mis.report.instance.period'].create(dict(
            name='p2',
            report_instance_id=self.instance.id,
    ...
    def test_sum(self):
        self.psum = self.env['mis.report.instance.period'].create(dict(
            name='psum',
            report_instance_id=self.instance.id,
            mode=MODE_NONE,
            source=SRC_SUMCOL,
            source_sumcol_ids=[
                (0, 0, dict(
                    period_to_sum_id=self.p1.id,
                    sign='+',
                )),
                (0, 0, dict(
                    period_to_sum_id=self.p2.id,
                    sign='+',
                )),
            ],
        ))
    ...
    def test_cmp(self):
        self.pcmp = self.env['mis.report.instance.period'].create(dict(
            name='pcmp',
            report_instance_id=self.instance.id,
            mode=MODE_NONE,
            source=SRC_CMPCOL,
