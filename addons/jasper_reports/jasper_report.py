...
class Report:
    def __init__(self, name, cr, uid, ids, data, context):
        ...
        self.output_format = self.data.get('output_format', False) or 'pdf'
    ...
    def execute(self):
        ...
        if self.data['output_format']:
            self.output_format = self.data['output_format']
        elif data['jasper_output']:
        ...
        fd, data_file = tempfile.mkstemp(suffix=".csv")
        os.close(fd)
        fd, output_file = tempfile.mkstemp(suffix=".csv")
        os.close(fd)

        if os.name == 'posix':
            os.chmod(data_file, 0666)
...
class ReportJasper(report.interface.report_int):
    ...
    def create(self, cr, uid, ids, datas, context=None):
        ...
        if self.parser:
            ...
            datas['output_format'] = d.get('output', 'pdf')
