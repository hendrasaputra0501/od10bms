...
class JasperServer:
    ...
    def start(self):
        ...
        env['CLASSPATH'] = os.path.join(self.path(), '..', 'java' + sep) + sep.join(
            glob.glob(libs)) + sep + os.path.join(self.path(), '..', 'custom_reports')
        ...
        command = ['java', '-Djava.awt.headless=true', '-Xms512M', '-Xmx1024M', 'com.nantic.jasperreports.JasperServer',
                   unicode(self.port)]
        # syntax for debug mode
        # command = ['java', '-Djava.awt.headless=true', '-Xms512M', '-Xmx1024M', '-Xdebug', '-Xrunjdwp:ransport=dt_socket,address=8000,server=y,suspend=n', 'com.nantic.jasperreports.JasperServer', unicode(self.port)]
