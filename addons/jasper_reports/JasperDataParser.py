...
REPORT_TYPE = [('pdf','PDF'), ('xls','XLS'), ('xlsx', 'XLSX'), ('csv','CSV'),
               ('odt','ODT'), ('ods','ODS'), ('txt','Text'), ('html','HTML'),
               ('jrprint', 'Jasper Print'), ('rtf','RTF'),]

def CONVERT_TUPLE(text=[]):
    return str(tuple(text)).replace(',)', ')')
...
class JasperDataParser(object):
    ...
    def get(self,parameter,default_value):
        if(parameter=="ids"):
            ...
            return hasattr(self, 'generate_output') and \
                   self.generate_output(self.cr, self.uid, self.ids, self.data, self.context) or default_value
